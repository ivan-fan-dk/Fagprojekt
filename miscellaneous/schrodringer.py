...
B = 1
x0 = 0
p = lambda x,t: A*(1/torch.cosh(B*(x-x0-v*t)))
phi_t = lambda x,t: a*x+b*t+c
u_sol_real = lambda x,t: p(x,t)*torch.cos(phi_t(x,t))
u_sol_img  = lambda x,t: p(x,t)*torch.sin(phi_t(x,t))
...
for epoch in range(num_epochs):
    ...
    loss_PDE_real = criterion(-du_dt_imag + d2u_dx2_real + 2*(u_real**2 + u_imag**2) * u_real, torch.zeros_like(u_real))
    loss_PDE_imag = criterion(du_dt_real + d2u_dx2_imag + 2*(u_real**2 + u_imag**2) * u_imag, torch.zeros_like(u_imag))
    ...
    loss = 1000*(adapt_weights[0] * loss_PDE  + adapt_weights[3]*loss_IC)
    #+ adapt_weights[1] * loss_boundary_1 + adapt_weights[2] * loss_boundary_2
...
plt.tight_layout()
plt.savefig("opgaver/_static/schrodinger.png")