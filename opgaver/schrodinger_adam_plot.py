...
state_dict = torch.load("schrodinger_model_from_hpc_plain_adam.pth", map_location=torch.device('cpu'))

# Print the keys and shapes in the state dictionary
for key, value in state_dict.items():
    print(f"{key}: {value.shape}")

#get model from HPC
model = NeuralNetwork()
model.load_state_dict(torch.load("schrodinger_model_from_hpc_plain_adam.pth", map_location=torch.device('cpu')))
model.eval()
######################################################################
############################# Plotting ###############################
######################################################################    
...
gs1.update(top=1, bottom=0.1, left=0.1, right=0.95, wspace=0.3)  # Adjusted bottom to create space for the legend
...
plt.savefig('NLS_plain_adam.png')