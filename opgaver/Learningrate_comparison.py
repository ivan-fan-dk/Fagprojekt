...
optimizer = torch.optim.Adam(model.parameters(), lr=0.001) #normal
...
for epoch in range(num_epochs_adam):
    ...
    loss.backward()
    optimizer.step()
    loss_bucket_large.append(loss.item())
    if (epoch + 1) % 1 == 0:
...
for epoch in range(num_epochs_adam):
    ...
    loss.backward()
    optimizer.step()
    loss_bucket_med.append(loss.item())
    if (epoch + 1) % 1 == 0:
        print(f'Epoch [{epoch+1}/{num_epochs_adam}], Loss: {loss.item():.4f}', end='\r')
...
for epoch in range(num_epochs_adam):
    ...
    loss.backward()
    optimizer.step()
    loss_bucket_small.append(loss.item())
    if (epoch + 1) % 1 == 0:
        print(f'Epoch [{epoch+1}/{num_epochs_adam}], Loss: {loss.item():.4f}', end='\r')
#lbfgs = optim.LBFGS(model.parameters(), max_iter=1)
#lbfgs.step(closure)
with torch.no_grad():
    plt.plot(list(np.arange(start=1,stop=num_epochs_adam+1)),loss_bucket_large,label = 'Learning rate = 0.001')
    plt.plot(list(np.arange(start=1,stop=num_epochs_adam+1)),loss_bucket_med,label = 'Learning rate = 0.0005')
    plt.plot(list(np.arange(start=1,stop=num_epochs_adam+1)),loss_bucket_small,label = 'Learning rate = 0.0001')
    plt.yscale('log')
    plt.legend()  # Add this line to display the legend
    plt.savefig("Adam_LearningRate.svg")
quit()
#save the model:
