...
N = int(200)
lambda_reg_small = 1e-4
lambda_reg_medium = 1e-3
...
for epoch in range(num_epochs_adam):
    ...
    loss.backward()
    optimizer.step()
    loss_bucket_no_reg.append(loss.item())
    if (epoch + 1) % 1 == 0:
...
#model.apply(NeuralNetwork.init_weights)
criterion = nn.MSELoss()
optimizer = torch.optim.Adam(model.parameters(), lr=0.001,weight_decay=lambda_reg_medium)
...
for epoch in range(num_epochs_adam):
    ...
    loss.backward()
    optimizer.step()
    loss_bucket_big_reg.append(loss.item())
    if (epoch + 1) % 1 == 0:
        print(f'Epoch [{epoch+1}/{num_epochs_adam}], Loss: {loss.item():.4f}', end='\r')
...
#model.apply(NeuralNetwork.init_weights)
criterion = nn.MSELoss()
optimizer = torch.optim.Adam(model.parameters(), lr=0.001,weight_decay=lambda_reg_small)
...
for epoch in range(num_epochs_adam):
    ...
    loss.backward()
    optimizer.step()
    loss_bucket_small_reg.append(loss.item())
    if (epoch + 1) % 1 == 0:
        print(f'Epoch [{epoch+1}/{num_epochs_adam}], Loss: {loss.item():.4f}', end='\r')
...
#lbfgs.step(closure)
with torch.no_grad():
    plt.plot(list(np.arange(start=1,stop=num_epochs_adam+1)),loss_bucket_small_reg,label = 'Regularization 1e-4')
    plt.plot(list(np.arange(start=1,stop=num_epochs_adam+1)),loss_bucket_big_reg,label = 'Regularization 1e-3')
    plt.plot(list(np.arange(start=1,stop=num_epochs_adam+1)),loss_bucket_no_reg,label = 'No Regularization')
    plt.yscale('log')
    plt.legend()  # Add this line to display the legend
    plt.savefig("Adam_Regularization.svg")
quit()
#save the model:
