...
#model.apply(NeuralNetwork.init_weights)
criterion = nn.MSELoss()
optimizer = torch.optim.Adam(model.parameters(), lr=0.001,weight_decay=lambda_reg)
...
for epoch in range(num_epochs_adam):
    ...
    loss.backward()
    optimizer.step()
    loss_bucket_reg.append(loss.item())
    if (epoch + 1) % 1 == 0:
        print(f'Epoch [{epoch+1}/{num_epochs_adam}], Loss: {loss.item():.4f}', end='\r')
...
#lbfgs.step(closure)
with torch.no_grad():
    plt.plot(list(np.arange(start=1,stop=num_epochs_adam+1)),loss_bucket_reg,label = 'Regularization')
    ...
    plt.savefig("Adam_Regularization.png")
quit()
#save the model:
